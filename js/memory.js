...
var game = {
    items: [], states: [], setValue: null, ready: 0, selectedCards: [], score: 200,
    pairs: 2, groupSize: 2, difficulty: 'normal', level: 1, isProcessing: false, turnsLeft: -1,
    goBack: function(idx){ this.setValue && this.setValue[idx](back); this.states[idx] = StateCard.ENABLE; },
    goFront: function(idx){ this.setValue && this.setValue[idx](this.items[idx]); this.states[idx] = StateCard.DISABLE; },
    saveScore: function() {
    ...
    select: function(){
        if (sessionStorage.load){
            ...
            this.items = toLoad.items; this.states = toLoad.states;
            this.selectedCards = toLoad.selectedCards || [];
            this.score = toLoad.score; this.pairs = toLoad.pairs;
            this.groupSize = toLoad.groupSize || 2; this.level = toLoad.level || 1;
            ...
            this.pairs = nextLvlData.pairs; this.groupSize = nextLvlData.groupSize;
            this.score = nextLvlData.score; this.difficulty = nextLvlData.difficulty;
            this.level = nextLvlData.level;
            let baseTime = this.difficulty === 'hard' ? 500 : (this.difficulty === 'easy' ? 1500 : 1000);
            if (baseTime - (this.level - 1) * 100 <= 200) this.turnsLeft = Math.ceil(this.pairs * 1.5);
            else this.turnsLeft = -1;
            sessionStorage.removeItem('mode2_next_level');
            this.items = resources.slice(); shuffe(this.items);
            let baseItems = this.items.slice(0, this.pairs);
            let totalItems = []; for (let i = 0; i < this.groupSize; i++) totalItems = totalItems.concat(baseItems);
            this.items = totalItems; shuffe(this.items); this.states = new Array(this.items.length);
        } else {
            this.level = 1; this.turnsLeft = -1;
            if (sessionStorage.getItem('mode') === '2') {
                this.pairs = 2; this.groupSize = 2; this.difficulty = 'easy';
            } else {
    ...
    start: function(){
        this.items.forEach((_, indx) => {
            if (this.states[indx] === StateCard.DONE || this.states[indx] === StateCard.DISABLE) {
                this.ready++; this.setValue && this.setValue[indx](this.items[indx]);
            } else {
                setTimeout(() => { this.ready++; this.goBack(indx); }, 1000 + 100 * indx);
            }
        });
    },
    click: function(indx){
        if (this.isProcessing || this.states[indx] !== StateCard.ENABLE || this.ready < this.items.length) return;
        this.goFront(indx); this.selectedCards.push(indx);
        if (this.selectedCards.length === this.groupSize) {
            if (this.turnsLeft > 0) this.turnsLeft--;
            let allMatch = true; let firstCardValue = this.items[this.selectedCards[0]];
            for (let i = 1; i < this.selectedCards.length; i++) if (this.items[this.selectedCards[i]] !== firstCardValue) { allMatch = false; break; }
            if (allMatch) {
                this.pairs--; if (sessionStorage.getItem('mode') === '2') this.score += (10 * this.level);
                this.selectedCards.forEach(idx => this.states[idx] = StateCard.DONE);
                if (this.pairs <= 0){
                    setTimeout(() => {
                        let currentMode = sessionStorage.getItem('mode') || '1';
                        if (currentMode === '1') { alert(`Guanyat: ${this.score}`); window.location.assign("../"); }
                            ...
                            sessionStorage.setItem('mode2_next_level', JSON.stringify({ level: this.level, pairs: this.pairs, groupSize: this.groupSize, score: this.score, difficulty: this.difficulty }));
                ...
                this.isProcessing = true; let cardsToHide = [...this.selectedCards];
                setTimeout(() => { cardsToHide.forEach(idx => this.goBack(idx)); this.isProcessing = false; }, hideTime);
                let penalty = 25 + ((this.level - 1) * 10); this.score -= penalty;
                if (this.score <= 0 || (this.turnsLeft === 0 && this.pairs > 0)){
                    this.saveScore();
                    setTimeout(() => { window.location.assign("../"); }, hideTime);
                }
            }
            this.selectedCards = [];
        }
    ...
    save: function(){
        let currentSave = {
            ...
            data: { items: this.items, states: this.states, selectedCards: this.selectedCards, score: this.score, pairs: this.pairs, groupSize: this.groupSize, difficulty: this.difficulty, level: this.level, turnsLeft: this.turnsLeft, mode: sessionStorage.getItem('mode') }
        };
        let saves = JSON.parse(localStorage.getItem('saves') || '[]');
        saves.push(currentSave);
        localStorage.setItem('saves', JSON.stringify(saves));
        alert("Guardat!"); window.location.assign("../");
    }
};
function shuffe(arr){ arr.sort(function () {return Math.random() - 0.5}); }
